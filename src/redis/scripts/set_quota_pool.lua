-- set_quota_pool.lua
-- KEYS: [pool_total, pool_available, pool_reserved, pool_loaned_out, pool_received]
...
local received = tonumber(redis.call('GET', pool_received) or '0')

if not new_total or new_total < 0 then
  return redis.error_reply('INVALID_TOTAL')
end

if (new_total + received) < (reserved + loaned_out) then
  return redis.error_reply('RESERVED_EXCEEDS_TOTAL')
end

-- Compute new available
local new_available = new_total - reserved - loaned_out + received
